;; load base configuration
(load-file "~/emacs/bk-base.el")

;; load theme
(load-theme 'default-black t)

(load-file "~/emacs/base/bk-base-completion.el")
(load-file "~/emacs/base/bk-base-git.el")
(load-file "~/emacs/base/bk-base-org.el")
(load-file "~/emacs/base/bk-base-package.el")
(load-file "~/emacs/base/bk-base-projectile.el")
(load-file "~/emacs/base/bk-base-settings.el")
(load-file "~/emacs/base/bk-base-ui.el")
(load-file "~/emacs/base/bk-base-programming.el")
(load-file "~/emacs/base/bk-base-lisps.el")
